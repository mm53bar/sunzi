...
module Snipe
  class Cli < Thor
    ...
    desc "deploy [environment] [user@host] [--sudo]", "Deploy snipe project"
    method_options :sudo => false
    def deploy(environment, target)
      do_deploy(environment, target, options.sudo?)
    end

    desc "compile", "Compile snipe project"
    def compile(environment)
      do_compile(environment)
    end

    no_tasks do
      include Snipe::Utility

      def self.source_root
        File.expand_path('../../',__FILE__)
      end

      def do_deploy(environment, target, force_sudo)
      ...
      def do_compile(environment)
        # Check if you're in the app directory
        ...
        Dir['.snipe/recipes/*'].each do |file|
